...
public class NBTUtils {
	...
	public static BlockPos getPos(final NBTTagCompound nbt) {
		if (nbt == null || !nbt.hasKey("x") || !nbt.hasKey("y") || !nbt.hasKey("z"))
			return null;
		if (nbt.hasKey("world"))
			return new WorldBlockPos(DimensionManager.getWorld(nbt.getInteger("world")),
					nbt.getDouble("x"), nbt.getDouble("y"), nbt.getDouble("z"));
		return new BlockPos(nbt.getDouble("x"), nbt.getDouble("y"), nbt.getDouble("z"));
	}

	public static NBTTagCompound getNBT(final BlockPos pos) {
		if (pos == null)
			return null;
		NBTTagCompound nbt = new NBTTagCompound();
		if (pos instanceof WorldBlockPos)
			nbt.setInteger("world", ((WorldBlockPos) pos).getWorld().provider.getDimension());
