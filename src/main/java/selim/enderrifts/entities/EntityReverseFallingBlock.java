...
//Code from EntityFallingBlock
public class EntityReverseFallingBlock extends Entity {
	...
	protected static final DataParameter<BlockPos> ORIGIN = EntityDataManager
			.<BlockPos>createKey(EntityReverseFallingBlock.class, DataSerializers.BLOCK_POS);

	public EntityReverseFallingBlock(World worldIn) {
		super(worldIn);
	}

	public EntityReverseFallingBlock(World worldIn, double x, double y, double z,
	...
	public void onUpdate() {
		...
		if (this.fallTile.getMaterial() == Material.AIR) {
			...
			if (!this.world.isRemote) {
				...
				boolean flag1 = flag
						&& this.world.getBlockState(blockpos1).getMaterial() == Material.WATER;
				double d0 = this.motionX * this.motionX + this.motionY * this.motionY
				...
				if (!this.onGround && !flag1) {
					if (this.fallTime > 100 && !this.world.isRemote
					...
					if (this.world.isAirBlock(
							new BlockPos(this.posX, this.posY + 0.009999999776482582D, this.posZ))) // Forge:
																									// Don't
																									// indent
																									// below.
						if (!flag1 && BlockFalling
								.canFallThrough(this.world.getBlockState(new BlockPos(this.posX,
										this.posY + 0.009999999776482582D, this.posZ)))) {
							this.onGround = false;
							return;
						}
					...
					if (iblockstate.getBlock() != Blocks.PISTON_EXTENSION) {
						...
						if (!this.dontSetBlock) {
							if (this.world.mayPlace(block, blockpos1, true, EnumFacing.UP, (Entity) null)
									&& (flag1 || !BlockFalling
											.canFallThrough(this.world.getBlockState(blockpos1.down())))
			...
			if (i > 0) {
				...
				for (Entity entity : list) {
					entity.attackEntityFrom(damagesource, (float) Math
							.min(MathHelper.floor((float) i * this.fallHurtAmount), this.fallHurtMax));
				}
				...
				if (flag && (double) this.rand.nextFloat() < 0.05000000074505806D + (double) i * 0.05D) {
					...
					if (j > 2) {
						...
						this.fallTile = this.fallTile.withProperty(BlockAnvil.DAMAGE,
								Integer.valueOf(j));
					}
				}
			}
	...
	public void addEntityCrashInfo(CrashReportCategory category) {
		...
		if (this.fallTile != null) {
			Block block = this.fallTile.getBlock();
			category.addCrashSection("Immitating block ID",
