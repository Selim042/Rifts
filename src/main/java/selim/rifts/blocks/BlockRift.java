...
import selim.rifts.world.WorldProviderRift;
...
public class BlockRift extends BlockContainer {
	...
	private boolean teleport(World world, BlockPos pos, IBlockState state, EntityPlayer player) {
		...
		if (player instanceof EntityPlayerMP)
			if (player.getEntityWorld().provider.getDimension() == DimensionRift.DIMENSION_ID)
				FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList()
						.transferPlayerToDimension((EntityPlayerMP) player,
								WorldProviderRift.getEntryDim(player),
								new RiftTeleporter(((EntityPlayerMP) player).getServerWorld()));
			else
				FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList()
