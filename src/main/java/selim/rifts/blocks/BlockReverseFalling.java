...
// Code from BlockFalling
public class BlockReverseFalling extends Block {

	public static boolean fallInstantly;

	public BlockReverseFalling() {
		super(Material.SAND);
		this.setCreativeTab(CreativeTabs.BUILDING_BLOCKS);
	}

	public BlockReverseFalling(Material materialIn) {
		super(materialIn);
	}
	...
	private void checkFallable(World worldIn, BlockPos pos) {
		if ((worldIn.isAirBlock(pos.up()) || canFallThrough(worldIn.getBlockState(pos.up())))
				&& pos.getY() <= worldIn.getHeight()) {
			int i = 32;

			if (!fallInstantly && worldIn.isAreaLoaded(pos.add(-32, -32, -32), pos.add(32, 32, 32))) {
				if (!worldIn.isRemote) {
					// Obviously changed EntityFallingBlock to
					// EntityReverseFallingBlock
					EntityReverseFallingBlock entityfallingblock = new EntityReverseFallingBlock(worldIn,
				...
				for (blockpos = pos.up(); (worldIn.isAirBlock(blockpos)
						|| canFallThrough(worldIn.getBlockState(blockpos)))
						&& blockpos.getY() > 0; blockpos = blockpos.up()) {
					;
				}
	...
	protected void onStartFalling(EntityReverseFallingBlock fallingEntity) {}
	...
	public void onEndFalling(World worldIn, BlockPos pos, IBlockState p_176502_3_,
